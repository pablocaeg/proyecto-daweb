...
const InfoRestaurante = ({ opinion, restaurant }) => {
  ...
  const nextImage = () => {
    setCarouselIndex((carouselIndex + 1) % restaurant.sitios[0].imagenes.length);
  }

  const prevImage = () => {
    setCarouselIndex((carouselIndex - 1 + restaurant.sitios[0].imagenes.length) % restaurant.sitios[0].imagenes.length);
  }
    ...
    <div className="info-restaurante-container">
      ...
      {restaurant.sitios.map((sitio, index) => (
        <div key={index} className="sitio-info">
            ...
            <button className="enlaces-button" onClick={() => setLinkListOpen(!linkListOpen)}>
              {linkListOpen ? "Esconder enlaces externos" : "Mostrar enlaces externos"}
            </button>
            {sitio.enlacesExternos?.length > 0 ? (linkListOpen && sitio.enlacesExternos.map((link, i) => <a key={i} href={link}>Enlace externo {i + 1}</a>)) : <p>Este sitio no tiene enlaces externos asociados.</p>}
          </div>

          <div className="sitio-imagenes">
            {sitio.imagenes?.length > 0 ? (
              <>
                <button className="imagenes-button" onClick={prevImage}>Siguiente</button>
                <img className="imagenes-display" src={sitio.imagenes[carouselIndex]} alt="Error al recuperar la imagen" />
                <button className="imagenes-button" onClick={nextImage}>Anterior</button>
              </>
            ) : (
